...
1. どこからも参照されていないデッドコードを削除し、保守対象を減らす
2. `docs/11-未実装機能一覧.md`の記載を実態(削除で対応)に合わせて更新する

### あえてやらなかったこと

| やらなかったこと | 理由 |
| --- | --- |
| `BudgetDonutChart`の実データ化 | `ExpensePieChart`と機能が重複するだけで、issueの本質(未使用コード)を解決しないため |
| 他の未使用コンポーネントの棚卸し | 今回のissueのスコープは`BudgetDonutChart`単体。他にも同様のデッドコードがないかの網羅的な調査は別issueとして扱う方が変更範囲が明確 |
...
このissue単体で完結する削除のため、追加の拡張候補は特にありません。強いて言えば、リポジトリ全体で他に同様の未使用コンポーネント・未使用exportがないか棚卸しする価値はありますが、今回のスコープ外です。

---

## よくある質問

### Q. `docs/11-未実装機能一覧.md`の他の項目もこの機会に見直した？

いいえ、今回のissue #37に対応する項目#20のみを更新しました。他の項目のステータスが実態とずれていないかは、レビューで指摘を受けて`推奨実装順序`セクションと`注意事項`セクションにも同じ項目#20への言及が残っていることに気づき、あわせて修正しています(表の行だけでなく、ドキュメント内の全参照箇所を更新する必要がありました)。

### Q. なぜ`ExpensePieChart`に統合しなかったの？

`ExpensePieChart`は既に`src/app/expense/page.tsx`で実データを使って動作しており、`BudgetDonutChart`と統合(コンポーネントを1つにまとめる)する積極的な理由がありませんでした。`BudgetDonutChart`はそもそもどこからも呼ばれていなかったため、「統合」ではなく単純な「削除」で問題は解決します。
