...
- リフレッシュトークンは **使うたびにローテーション**(古いものを「使用済み」にマークし新しいものを発行)し、再利用(リプレイ)を防ぐ。この「まだ未使用なら失効させる」判定は`updateMany`のWHERE条件付き更新1文でアトミックに行い、同時リクエストによる二重発行・誤検知を防いでいる
...
- スライディングウィンドウ(30日、ローテーションのたび延長)とは別に、**最初のログインから90日を超えたら無条件で再ログインを要求する絶対有効期限**を設けた
- `POST /api/auth/refresh`にも`login`/`register`と同様のIP単位レート制限を追加した
